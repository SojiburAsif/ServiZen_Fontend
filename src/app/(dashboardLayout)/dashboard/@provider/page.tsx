import { getStats, ProviderStats } from "@/services/stats.service";
import {
  Wallet,
  TrendingUp,
  Star,
...
function BarChart({ data, maxVal, color = "from-emerald-600 to-emerald-400" }: { data: { label: string; value: number }[]; maxVal: number; color?: string }) {
  return (
...
export default async function ProviderDashboardPage() {
  ...
  if (!raw || raw.role !== "PROVIDER") {
    return (
      <div className="flex h-64 items-center justify-center">
        <p className="text-slate-400 text-sm">Could not load provider stats. Please refresh.</p>
      </div>
    );
  }

  const stats = raw as ProviderStats;
  ...
  const rating = overview.averageRating ?? 0;
    ...
    <div className="space-y-8 p-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500 dark:text-zinc-400">
          Your earnings, services & booking overview
        </p>
      </div>

      {/* Wallet & Rating Banner */}
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="rounded-2xl bg-gradient-to-br from-emerald-600 to-teal-600 p-6 text-white shadow-lg shadow-emerald-500/20">
          <p className="text-xs font-bold uppercase tracking-widest text-emerald-100">Wallet Balance</p>
          <p className="mt-2 text-4xl font-black">৳{(overview.walletBalance ?? 0).toLocaleString()}</p>
          <p className="mt-1 text-sm text-emerald-200">Total Earned: ৳{(overview.totalEarned ?? 0).toLocaleString()}</p>
        </div>
        <div className="rounded-2xl bg-gradient-to-br from-yellow-500 to-orange-500 p-6 text-white shadow-lg shadow-yellow-500/20">
          ...
          <div className="mt-2 flex items-end gap-2">
            ...
            <div className="mb-1 flex gap-0.5">
              {[1, 2, 3, 4, 5].map((s) => (
                <Star key={s} size={14} className={s <= Math.round(rating) ? "fill-white text-white" : "text-yellow-200"} />
              ))}
            </div>
          </div>
          <p className="mt-1 text-sm text-yellow-200">{overview.totalReviews ?? 0} reviews</p>
        </div>
      </div>

      {/* Overview Cards */}
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <StatCard label="Total Services" value={overview.totalServices ?? 0} icon={Wrench} color="bg-violet-500" />
        <StatCard label="Active Services" value={overview.activeServices ?? 0} icon={CheckCircle} color="bg-emerald-500" />
        <StatCard label="Total Bookings" value={overview.totalBookings ?? 0} icon={BookOpen} color="bg-blue-500" />
        <StatCard label="Reviews" value={overview.totalReviews ?? 0} icon={MessageSquare} color="bg-pink-500" />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {monthlyIncome && monthlyIncome.length > 0 && (
          <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-sm font-black uppercase tracking-widest text-slate-500 dark:text-zinc-400">Monthly Income</h2>
              <div className="flex items-center gap-1.5 rounded-full bg-emerald-50 px-3 py-1 dark:bg-emerald-900/20">
                <TrendingUp size={12} className="text-emerald-600" />
                <span className="text-[10px] font-bold text-emerald-600">Last 12 Months</span>
              </div>
            </div>
            <BarChart
        ...
        {weeklyIncome && weeklyIncome.length > 0 && (
          <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-sm font-black uppercase tracking-widest text-slate-500 dark:text-zinc-400">Weekly Income</h2>
              <div className="flex items-center gap-1.5 rounded-full bg-blue-50 px-3 py-1 dark:bg-blue-900/20">
                <Activity size={12} className="text-blue-600" />
                <span className="text-[10px] font-bold text-blue-600">Last 4 Weeks</span>
              </div>
            </div>
            <BarChart
      ...
      {recentBookings && recentBookings.length > 0 && (
        <div className="rounded-2xl border border-slate-100 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900 overflow-hidden">
          <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4 dark:border-zinc-800">
            <h2 className="text-sm font-black uppercase tracking-widest text-slate-500 dark:text-zinc-400">Recent Bookings</h2>
              ...
              <thead>
                <tr className="border-b border-slate-50 dark:border-zinc-800/50">
                  {["Service", "Client", "Date", "Amount", "Status"].map((h) => (
                    <th key={h} className="px-4 py-3 text-left text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-zinc-500">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {recentBookings.map((b) => (
                  <tr key={b.id} className="border-b border-slate-50 hover:bg-slate-50/50 dark:border-zinc-800/50 dark:hover:bg-zinc-800/30">
                    ...
                    <td className="px-4 py-3.5 text-slate-500 dark:text-zinc-500 text-xs">{new Date(b.bookingDate).toLocaleDateString()}</td>
